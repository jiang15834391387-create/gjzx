...
public class DataCustomsExportGoodsConversion implements ChangeIdentifyInfo<List<IdentifyResults>> {
    ...
    private static class LazyHolder {
        ...
        private static final DataCustomsExportGoodsConversion INSTANCE = new DataCustomsExportGoodsConversion();
    }

    private DataCustomsExportGoodsConversion() {
    }

    public static DataCustomsExportGoodsConversion getInstance() {
        return DataCustomsExportGoodsConversion.LazyHolder.INSTANCE;
    }
    ...
    public List<IdentificationData> changeInfo(DataImageFilesInfo dataImageFilesInfo, List<IdentifyResults> identifyResult) throws OcrException {
        ...
        for (IdentifyResults identifyResults : identifyResult) {
            ...
            DataCustomsExportGoods invoice = new DataCustomsExportGoods();
            if (null == jsonObject) {
                return null;
            }
            ...
            invoice.setDateOfExport(jsonObject.getStr("date_of_export"));
            ...
            invoice.setOverseasConsigneeCode(jsonObject.getStr("overseas_consignee_code"));
            invoice.setOverseasConsigneeName(jsonObject.getStr("overseas_consignee_name"));
            invoice.setPortOfExportCode(jsonObject.getStr("port_of_export_code"));
            invoice.setPortOfExportName(jsonObject.getStr("port_of_export_name"));
            ...
            List<DataCustomsExportGoodsDetail> ocrDetailsList = new ArrayList<>();
            if (ObjectUtil.isNotNull(list)) {
                for (Object invoiceDetails : list) {
                    DataCustomsExportGoodsDetail e = new DataCustomsExportGoodsDetail();
                    ...
                    e.setOriginalPlaceOfDeliveredGoods(fJson.containsKey("original_place_of_delivered_goods") ? fJson.get("original_place_of_delivered_goods") : null);
            ...
            dataImageFilesInfo.setInvoice(InvoiceGlorityEnumd.CUSTOMS_EXPORT_GOODS_CODE.getCode());
            dataImageFilesInfo.setMessage(identifyResults.getMessage());
            resultsList.add(new IdentificationData<>(InvoiceGlorityEnumd.CUSTOMS_EXPORT_GOODS_CODE.getCode(), invoice, identifyResults.getExtra(), identifyResults.getMessage()));
        }
        return resultsList;
    }
