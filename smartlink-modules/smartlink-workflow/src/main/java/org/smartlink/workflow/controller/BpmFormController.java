...
public class BpmFormController {
    ...
    public R<Long> createForm(@Valid @RequestBody BpmFormSaveReqVO createReqVO) {
    ...
    public R<Boolean> updateForm(@Valid @RequestBody BpmFormSaveReqVO updateReqVO) {
    ...
    public R<BpmFormRespVO> getForm(@RequestParam("id") Long id) {
        BpmFormDO form = formService.getForm(id);
        return R.ok(MapstructUtils.convert(form, BpmFormRespVO.class));
    }

    @GetMapping({"/list-all-simple", "/simple-list"})
    @Operation(summary = "获得动态表单的精简列表", description = "用于表单下拉框")
    public R<List<BpmFormDO>> getFormSimpleList() {
    ...
    @SaCheckPermission("bpm:form:query")
    public TableDataInfo<BpmFormRespVO> getFormPage(@Valid BpmFormRespVO pageVO, PageQuery pageQuery) {
