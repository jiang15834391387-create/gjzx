package org.smartlink.server.nc.utils.file;
...
import org.smartlink.server.task.util.TiffToJpgUtils;
...
public class FileUtils extends FileUtil {
    ...
    public static byte[] downloadFile(String fileUrl) throws IOException {
        if (fileUrl.contains(Constants.RESOURCE_PREFIX)) {
            fileUrl = fileUrl.replace(Constants.RESOURCE_PREFIX, RedisUtils.getCacheObject(Constants.SYS_CONFIG_KEY + ParamConstants.SYS_LOCAL_LOCATION));
     ...
     * @throws IOException
     */
    public static byte[] downloadFile(String fileUrl, String fileId) throws IOException {
        if (fileUrl.contains(Constants.RESOURCE_PREFIX)) {
            fileUrl = fileUrl.replace(Constants.RESOURCE_PREFIX, RedisUtils.getCacheObject(Constants.SYS_CONFIG_KEY + ParamConstants.SYS_LOCAL_LOCATION));
            fileUrl = fileUrl.substring(0, fileUrl.lastIndexOf(File.separator) + 1) + fileId;
            log.info(fileId + "下载路径：" + fileUrl);
            return FileUtil.readBytes(fileUrl);
        } else {
            return IOUtils.toByteArray(new URL(fileUrl));
        }
     ...
     * @throws IOException
     */
    public static byte[] downloadDocuments(String fileUrl, String fileId) throws IOException {
        if (fileUrl.contains(Constants.RESOURCE_PREFIX)) {
            fileUrl = fileUrl.replace(Constants.RESOURCE_PREFIX, RedisUtils.getCacheObject(Constants.SYS_CONFIG_KEY + ParamConstants.SYS_LOCAL_LOCATION));
            log.info(fileId + "下载路径：" + fileUrl);
            return FileUtil.readBytes(fileUrl);
     ...
     * @return InputStream
     */
    public static String getFileUrl(String fileUrl, String fileId) {
        if (fileUrl.contains(Constants.RESOURCE_PREFIX)) {
            fileUrl = fileUrl.replace(Constants.RESOURCE_PREFIX, RedisUtils.getCacheObject(Constants.SYS_CONFIG_KEY + ParamConstants.SYS_LOCAL_LOCATION));
            fileUrl = fileUrl.substring(0, fileUrl.lastIndexOf(File.separator) + 1) + fileId;
        } else {
