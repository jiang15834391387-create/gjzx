...
import org.smartlink.server.nc.constant.OcrConstant;
import org.smartlink.server.nc.ocr.enumd.FactoryEnum;
import org.smartlink.server.nc.ocr.enumd.OcrEnumd;
...
public class OcrFactory {
     ...
     * 根据类型获取实例
     */
    private static IOcrStrategy instance(String type) {
    ...
    public static Class<?> instanceObj() {
        ...
        FactoryEnum factoryEnumd = FactoryEnum.find(type);
    ...
    private static AbstractOcrStrategy getStrategy(String type) {
        OcrEnumd checkEnum = OcrEnumd.find(type);
        if (checkEnum == null) {
            throw new OcrException("识别服务类型无法找到!");
        }
